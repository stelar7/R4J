...
public class DataCall {
	...
	public enum ResponseType {
		OK, USER_ERROR, LIBRARY_ERROR, SERVER_ERROR;
	}

	public static class DataCallBuilder {
		private final DataCall dc = new DataCall();

		private BiFunction<String, String, String> merge = ((o, n) -> o + "," + n);
		...
		public DataCallBuilder withURLData(String key, String value) {
			try {
				dc.urlData.merge(key, URLEncoder.encode(value, "UTF-8"), merge);
		...
		public DataCallBuilder withURLParameter(String key, String value) {
			try {
				dc.urlParams.merge(key, URLEncoder.encode(value, "UTF-8"), merge);
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			return this;
		...
		public Pair<ResponseType, Object> build() {
			...
			try {
				...
				if (con.getResponseCode() != 200) {
					...
					if (con.getResponseCode() == 429) {
						if (dc.retry) {
							...
							if (limitType != null
									&& (limitType.equals(LIMIT_USER) || limitType.equals(LIMIT_SERVICE))) {
						...
						con.disconnect();
						return new Pair<>(
								con.getResponseCode() < 499 ? ResponseType.USER_ERROR : ResponseType.SERVER_ERROR,
								new APIError(con.getResponseCode()));
					}
				}
				...
				Object dtoobj = ((APIObject) dc.endpoint.type.newInstance()).createFromString(dc.endpoint.type,
						data.toString());
